...
void rotate_canvas(lv_obj_t *canvas, uint8_t cbuf[]) {
  ...
  const int32_t stride =
      lv_draw_buf_width_to_stride(CANVAS_HEIGHT, CANVAS_COLOR_FORMAT);
  lv_draw_sw_rotate(cbuf_tmp, cbuf, CANVAS_HEIGHT, CANVAS_HEIGHT, stride,
                    stride, LV_DISPLAY_ROTATION_270, CANVAS_COLOR_FORMAT);
}
...
void canvas_draw_rect(lv_obj_t *canvas, int32_t x, int32_t y, int32_t w,
                      int32_t h, lv_draw_rect_dsc_t *rect_dsc) {
  lv_layer_t layer;
...
void canvas_draw_img(lv_obj_t *canvas, int32_t x, int32_t y, const void *src,
  ...
  img_dsc->src = img_src;
  lv_area_t coords = {x, y, x + img_src->header.w - 1,
                      y + img_src->header.h - 1};
